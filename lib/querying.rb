...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM series INNER JOIN subgenres ON subgenres.id = series.subgenre_id INNER JOIN authors ON authors.id = series.author_id;"
end
...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series
    ...
    GROUP BY series.title HAVING characters.species = 'human' ORDER BY COUNT(series.title) LIMIT 1;
    "
end
